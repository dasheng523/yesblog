...
-}
module Ys.Yampa.Elevator where
...
elevator = proc (lbp, rbp) -> do
    -- proc 为了让复杂的 Arrow 计算更易读
    rec -- This delayed hold can be thought of as modelling acceleration.
        ...
        -- lMerge 做合并两个事件信号，如果同时发生，取左边的信号
        -- gate 根据布尔条件启用或禁用事件发生
        ...
        let setUp = lbp `tag` True -- tag 是一个用于给事件附加新值的函数。
